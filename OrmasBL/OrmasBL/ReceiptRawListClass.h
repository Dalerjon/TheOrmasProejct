...
namespace BusinessLayer
{
	...
	protected:
		...
		int currencyID = 0;
	public:
		ReceiptRawList(int rlID, int rpID, int pID, int rlCount, double rlSum, int sID, int cID) :id(rlID), receiptRawID(rpID),
		...
		bool CreateReceiptRawList(DataLayer::OrmasDal& ormasDal, int rID, int pID, int rlCount, double rlSum,
			int sID, int cID, std::string& errorMessage);
		bool UpdateReceiptRawList(DataLayer::OrmasDal& ormasDal, int rID, int pID, int rlCount, double rlSum,
			int sID, int cID, std::string& errorMessage);
