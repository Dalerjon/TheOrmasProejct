...
namespace BusinessLayer
{
	...
	protected:
		...
		int currencyID = 0;
	public:
		WriteOffRawList(int wlID, int wID, int pID, int wlCount, double wlSum, int sID, int cID) :id(wlID), writeOffRawID(wID),
		...
		bool CreateWriteOffRawList(DataLayer::OrmasDal& ormasDal, int wID, int pID, int wlCount, double wlSum,
			int sID, int cID, std::string& errorMessage);
		bool UpdateWriteOffRawList(DataLayer::OrmasDal& ormasDal, int wID, int pID, int wlCount, double wlSum,
			int sID, int cID, std::string& errorMessage);
